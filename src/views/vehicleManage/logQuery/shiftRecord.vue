<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-input v-model="listQuery.operatorName" placeholder="操作员名称" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
    ...
    <el-table v-loading="listLoading" :key="tableKey" :data="list" border fit highlight-current-row style="width: 100%;min-height:500px;">
      <el-table-column label="岗亭名称" width="80px">
        <template slot-scope="scope">
          <span>{{ scope.row.postName }}</span>
        </template>
      </el-table-column>
      <el-table-column label="应收金额" class-name="status-col" width="100">
        <template slot-scope="scope">
          <span>{{ scope.row.receivableAmount }}</span>
        </template>
      </el-table-column>
      <el-table-column label="优惠金额" width="110px" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column label="接班时间" class-name="status-col" width="100">
        <template slot-scope="scope">
          <span>{{ scope.row.shiftChangeTime | parseTime('{y}-{m}-{d} {h}:{i}:{s}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="交班人" class-name="status-col" width="100">
        <template slot-scope="scope">
          <span>{{ scope.row.shiftGiver }}</span>
        </template>
      </el-table-column>
      <el-table-column label="交班时间" class-name="status-col" width="100">
        <template slot-scope="scope">
          <span>{{ scope.row.shiftGiverWorkTime | parseTime('{y}-{m}-{d} {h}:{i}:{s}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="接班人" class-name="status-col" width="100">
        <template slot-scope="scope">
          <span>{{ scope.row.shiftTaker }}</span>
        </template>
      </el-table-column>
      <el-table-column label="异常备注" >
        <template slot-scope="scope">
          <span>{{ scope.row.abnormalRemark }}</span>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      fetchShiftChangeList(this.listQuery).then(response => {
        if (response.status === 200) {
          if (response.data.code === 200) {
            this.list = response.data.data.shiftChangeInfoElementList
