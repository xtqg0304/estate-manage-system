<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :key="tableKey" :data="list" border fit highlight-current-row style="width: 100%;min-height:500px;">
      ...
      <el-table-column label="进出记录ID" min-width="110px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.estateName }}</span>
        </template>
      </el-table-column>
      <el-table-column label="车牌" width="110px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.houseHoldName }}</span>
        </template>
      </el-table-column>
      <el-table-column label="进场时间" width="80px">
        <template slot-scope="scope">
          <span>{{ scope.row.payType | payFilter }}</span>
        </template>
      </el-table-column>
      <el-table-column label="出厂时间" align="center" width="95">
        <template slot-scope="scope">
          <span>{{ scope.row.payAmount }}</span>
        </template>
      </el-table-column>
      <el-table-column label="进场通道" class-name="status-col" width="100">
        <template slot-scope="scope">
          <span>{{ scope.row.payStatus | statusFilter }}</span>
        </template>
      </el-table-column>
      <el-table-column label="出场通道" class-name="status-col" width="100">
        <template slot-scope="scope">
          <span>{{ scope.row.payStatus | statusFilter }}</span>
        </template>
      </el-table-column>
      <el-table-column label="电子收费" class-name="status-col" width="100">
        <template slot-scope="scope">
          <span>{{ scope.row.payStatus | statusFilter }}</span>
        </template>
      </el-table-column>
      <el-table-column label="现金支付" class-name="status-col" width="100">
        <template slot-scope="scope">
          <span>{{ scope.row.payStatus | statusFilter }}</span>
        </template>
      </el-table-column>
      <el-table-column label="收费备注" class-name="status-col" width="100">
        <template slot-scope="scope">
          <span>{{ scope.row.payStatus | statusFilter }}</span>
        </template>
      </el-table-column>
