<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-select v-model="listQuery.payCategory"
                 placeholder="支付类目"
                 clearable
                 class="filter-item">
    ...
    <el-table v-loading="listLoading"
      ...
      <el-table-column label="支付类目"
                       width="100">
        <template slot-scope="scope">
          <span>{{ scope.row.payCategory | typeFilter }}</span>
        </template>
      </el-table-column>
      <el-table-column label="支付月份"
                       width="100">
        <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    /* 导出详情 */
    handleDownload() {
      const url = `http://${window.location.host}/api/payment/property/detail/exportOrderDetailExcel?communityId=${this.communityId}&beginTime=${this.listQuery.beginTime}&endTime=${this.listQuery.endTime}&payType=${this.listQuery.payType}&payCategory=${this.listQuery.payCategory}&searchKey=${this.listQuery.searchKey}`
